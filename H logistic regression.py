...
#accuracy=71/84= 0.8452380952380952
#---------------------------------logistic reg end-------------------------------------------


#logestic regression is most accurate compared to other methods
#now lets work on testing data
hungarian = (pd.read_table('/Users/bhaskaryuvaraj/Documents/data_science work_files/Hungarian Data.txt',
                       sep=',', names=header_row))
hungarian.dtypes
#since many columns have dtype object convert it into int and float
hungarian['BP']=pd.to_numeric(hungarian['BP'],errors='coerce')
hungarian['chol']=pd.to_numeric(hungarian['chol'],errors='coerce')
hungarian['fbs']=pd.to_numeric(hungarian['fbs'],errors='coerce')
hungarian['ecg']=pd.to_numeric(hungarian['ecg'],errors='coerce')
hungarian['maxhr']=pd.to_numeric(hungarian['maxhr'],errors='coerce')
hungarian['eiang']=pd.to_numeric(hungarian['eiang'],errors='coerce')
hungarian['slope']=pd.to_numeric(hungarian['slope'],errors='coerce')
hungarian['vessels']=pd.to_numeric(hungarian['vessels'],errors='coerce')
hungarian['thal']=pd.to_numeric(hungarian['thal'],errors='coerce')

#to find null values and treat
hungarian.isnull().sum()
hungarian=hungarian.dropna()
hungarian.drop('diagnosis',inplace=True,axis=1)

#now testing the data

pred_hungarian=logreg.predict(hungarian)
#result diagnosis=1 which is accurate
#---------------------------------------end of testing----------------------------------------------
